...
public sealed class SnapshotService
{
    ...
    public IReadOnlyList<PreservedFile> PreserveChangedFiles(SnapshotDiff diff, string caseId, string generation = "after")
    {
    ...
    private static IEnumerable<FileSnapshotEntry> CaptureFiles(string root)
    {
        ...
        while (stack.Count > 0 && visited < 5000)
        {
            ...
            foreach (string file in files)
            {
                ...
                yield return new FileSnapshotEntry(info.FullName, info.Length, info.LastWriteTimeUtc, hash, info.Attributes.ToString(), info.CreationTimeUtc, executable, anomaly, CaptureAlternateStreams(info.FullName));
            }
    ...
    private static IReadOnlyList<string> CaptureAlternateStreams(string path)
    {
        if (!OperatingSystem.IsWindows()) return [];
        var streams = new List<string>(); IntPtr handle = FindFirstStreamW(path, 0, out var data, 0);
        if (handle == new IntPtr(-1)) return [];
        try { do { if (!data.Name.Equals("::$DATA", StringComparison.OrdinalIgnoreCase)) streams.Add(data.Name); } while (streams.Count < 32 && FindNextStreamW(handle, out data)); }
    ...
    private static IReadOnlyList<RegistrySnapshotEntry> CaptureRegistry()
    {
        ...
        CaptureRegistryValues(values, Registry.CurrentUser, @"Software\Microsoft\Windows NT\CurrentVersion\Windows", @"HKCU\Software\Microsoft\Windows NT\CurrentVersion\Windows");
        CaptureRegistryValues(values, Registry.LocalMachine, @"Software\Microsoft\Windows\CurrentVersion\Run", @"HKLM\Software\Microsoft\Windows\CurrentVersion\Run");
        CaptureRegistryValues(values, Registry.LocalMachine, @"Software\Microsoft\Windows\CurrentVersion\RunOnce", @"HKLM\Software\Microsoft\Windows\CurrentVersion\RunOnce");
        CaptureRegistryValues(values, Registry.LocalMachine, @"Software\Microsoft\Windows NT\CurrentVersion\Winlogon", @"HKLM\Software\Microsoft\Windows NT\CurrentVersion\Winlogon");
        CaptureRegistryTree(values, RegistryHive.LocalMachine, RegistryView.Registry64, @"Software\Microsoft\Windows NT\CurrentVersion\Image File Execution Options", "HKLM64", 2);
        CaptureRegistryTree(values, RegistryHive.LocalMachine, RegistryView.Registry32, @"Software\Microsoft\Windows NT\CurrentVersion\Image File Execution Options", "HKLM32", 2);
        CaptureRegistryTree(values, RegistryHive.LocalMachine, RegistryView.Registry64, @"SYSTEM\CurrentControlSet\Services", "HKLM64", 2);
        CaptureRegistryTree(values, RegistryHive.LocalMachine, RegistryView.Registry64, @"Software\Microsoft\Windows NT\CurrentVersion\Schedule\TaskCache\Tasks", "HKLM64", 1);
        CaptureRegistryTree(values, RegistryHive.LocalMachine, RegistryView.Registry64, @"Software\Classes\CLSID", "HKLM64", 1);
        CaptureRegistryTree(values, RegistryHive.CurrentUser, RegistryView.Default, @"Software\Microsoft\Wbem\CIMOM", "HKCU", 1);
    ...
    [SupportedOSPlatform("windows")]
    private static void CaptureRegistryTree(List<RegistrySnapshotEntry> values, RegistryHive hive, RegistryView view, string subKey, string displayRoot, int depth)
    {
        try
        {
            ...
            {
                if (values.Count >= 20_000) return;
                using var key = root.OpenSubKey(path, false); if (key is null) return;
                string display = $"{displayRoot}\\{path}";
                foreach (string name in key.GetValueNames().Take(256)) values.Add(new RegistrySnapshotEntry(display, name, Convert.ToString(key.GetValue(name, null, RegistryValueOptions.DoNotExpandEnvironmentNames), System.Globalization.CultureInfo.InvariantCulture) ?? ""));
