...
public sealed class CaseRunner
{
    ...
    public StaticAnalysisResult Static(string targetPath, string outputRoot, string formats, string? caseName = null)
    {
        ...
        File.WriteAllText(Path.Combine(directory, "sample_metadata.json"), System.Text.Json.JsonSerializer.Serialize(result, JsonDefaults.Options));
        File.WriteAllText(Path.Combine(directory, "sections.csv"), "name,virtual_address,raw_size,entropy\r\n" + string.Join("\r\n", result.Sections.Select(s => $"{s.Name},{s.VirtualAddress},{s.RawSize},{s.Entropy}")));
        File.WriteAllText(Path.Combine(directory, "imports.csv"), "name\r\n" + string.Join("\r\n", result.Imports.Select(Csv)));
        File.WriteAllText(Path.Combine(directory, "exports.csv"), "name\r\n" + string.Join("\r\n", result.Exports.Select(Csv)));
        File.WriteAllText(Path.Combine(directory, "strings.csv"), "classification,value\r\n" + string.Join("\r\n", result.SuspiciousStrings.Select(s => $"{Csv(ClassifyString(s))},{Csv(s)}")));
        File.WriteAllText(Path.Combine(directory, "strings.json"), System.Text.Json.JsonSerializer.Serialize(result.SuspiciousStrings, JsonDefaults.Options));
        if (formats.Contains("html", StringComparison.OrdinalIgnoreCase) || formats.Contains("all", StringComparison.OrdinalIgnoreCase))
        {
            var data = DemoCaseFactory.Create(targetPath, result, 0);
            new CaseExportService().WriteCaseBundle(data, directory, "html,json,csv");
