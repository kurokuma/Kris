...
- hosts、WinINet/WinHTTP proxy、Explorer、Defender、Firewall、Security Centerを対象とする読み取り専用のbefore/afterホスト設定スナップショット
  - 変更イベントはsurface・キー/値名・変更前後の正規化値を記録し、Privacy Modeでは通常のケース出力と同様にマスクします。
  - レジストリ列挙は名前・値ともに上限付きです。列挙上限、値サイズ上限、読取り中の再サイズを Collection Quality に記録し、部分値を保持しません。
...
画面上のライブ履歴も10,000件に制限され、古い表示イベントを破棄した数は画面に表示されます。最終ケースは収集器が先着保持したイベント列から確定し、永続収集の上限到達・欠落数・理由はCollection Qualityに記録されます。
...
Command chains are normalized read-only. MRTW recognizes PowerShell `-EncodedCommand` / `-enc`, `FromBase64String`, and `powershell`, `pwsh`, `certutil`, `rundll32`, `regsvr32`, and `mshta`. Base64 handling is bounded (16 KiB input and 32 KiB decoded text), has decode depth zero (no recursive decoding), records limits and failures, and never launches a decoder, shell, COM component, archive, file write, or network operation. Commands and event evidence appear in the GUI and JSON/CSV/HTML/SQLite/ZIP exports; Privacy Mode redacts them through the shared export path.
