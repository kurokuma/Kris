...
- 完了条件:
  - [ ] AMSI/PowerShell ScriptBlock/レジストリ/ファイルI/OのETWをroot/子孫PIDに限定して収集し、面別の上限・欠落・非対応をCollection Qualityへ記録する
  - [ ] ScriptBlock/AMSIの実行時内容を候補長・件数上限付きで保持し、PID bind前は保持しないraw ETL境界と既存イベント順序・出力互換性を維持する
  - [ ] Hook無効・非対応時でもスクリプト・レジストリ・ファイルの逐次証拠が得られ、Runtime/Snapshot経路と二重計上しない
  - [ ] SafeRuntimeProbe拡張で作成/変更/ScriptBlock観測、Privacy Mode、上限超過、非管理者での`unavailable`区別の回帰テストが成功する
...
- 完了条件:
  - [ ] PID/ParentPIDから決定的にMermaid/DOTのプロセスツリーを生成し、上限超過を注記する
  - [ ] ラベルを安全にエスケープし、report.htmlへ埋め込みつつ単体ファイルも出力する
  - [ ] 循環参照・PID再利用・孤立ノード・空ケースを安全に扱う回帰テストが成功する
- 依存関係: なし
