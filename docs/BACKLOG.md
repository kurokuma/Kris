...
- 完了条件:
  - [x] mutex/pipe/service/task/imphash/PDB等を決定的に正規化・重複排除し、原表記と全source/process/evidence IDsを保持する
  - [x] 静的・動的・スナップショット結果から同一規則でホストIOCを生成し、全形式・Privacy Modeで往復する
  - [x] 同値異表記、host-wide snapshot attribution、上限、Privacy、SQLite/JSON再読込を含むoffline回帰テストが成功する
- 依存関係: TASK-016
